...
const { Fynox } = require('../../structures/Fynox')
module.exports = {
    name: 'whitelist',
    aliases: ['wl'],
                ...
                msg = await message.channel.send({
                    embeds: [
                        new MessageEmbed()
                            ...
                            .setColor(client.color)
                            .setDescription(
                                `${data.ban ? ' ' : '  '}: **Ban**\n${data.kick ? ' ' : '  '}: **Kick**\n${data.prune ? ' ' : '  '}: **Prune**\n${data.botadd ? ' ' : '  '}: **Bot Add**\n${data.serverup ? ' ' : '  '}: **Server Update\n${data.memup ? ' ' : '  '}: Member Role Update**\n${data.chcr ? ' ' : '  '}: **Channel Create**\n${data.chdl ? ' ' : '  '}: ** Channel Delete**\n${data.chup ? ' ' : '  '}: **Channel Update**\n${data.rlcr ? ' ' : '  '}: **Role Create**\n${data.rldl ? ' ' : '  '}: **Role Delete**\n${data.rlup ? ' ' : '  '}: **Role Update**\n${data.meneve ? ' ' : '  '}: **Mention** @everyone\n${data.mngweb ? ' ' : '  '}: **Webhook Management**\n${data.mngstemo ? ' ' : '  '}: **Emojis & Stickers Management**`
                            )
                            .addFields(
                                {
                                    name: `**Executor**`,
