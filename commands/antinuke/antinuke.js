        ...
        if (!option) {
            ...
            if (isActivatedAlready) {
                ...
                if (message.guild.roles.cache.size > 249)
                    return message.reply(`I Won't Able To Create \`Fynox Untouchable Shield\` Cause There Are Already 249 Roles In This Server`);
                ...
                let createdRole = await message.guild.roles.create({
                    name: 'Fynox Untouchable Shield',
                    position: role ? role : 0,
                    reason: 'Fynox Role For Ubypassable Setup',
            ...
            if (!isActivatedAlready) {
                ...
                try {
                    const FynoxRole = message.guild.roles.cache.find(role => role.name === 'Fynox Untouchable Shield');
                    if (FynoxRole) {
                        await FynoxRole.delete('Antinuke module disabled');
                    }
                } catch (error) {
                    console.error('Error deleting Fynox Untouchable Shield role:', error);
                    await message.channel.send({
                        embeds: [
                            new MessageEmbed()
                                .setColor(client.color)
                                .setDescription(`${client.emoji.cross} Couldn't delete the Fynox Untouchable Shield role. Please check my permissions.`)
                        ]
                    });
                }
