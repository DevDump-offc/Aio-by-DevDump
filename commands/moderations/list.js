                    ...
                    collector.on('collect', async (interaction) => {
                        ...
                        if (interaction.customId === 'admin_bots') {
                            ...
                            items = message.guild.members.cache
                                .filter(member => member.permissions.has('ADMINISTRATOR', true) && member.user.bot)
                            ...
                            items = message.guild.roles.cache
                                .filter(role => {
                                    ...
                                    return role.permissions.has('ADMINISTRATOR', true) && !isIntegrationRole;
                        ...
                        paginate(
                            message,
                            items,
                            interaction.customId === 'admin_roles'
                                ? 'Administrator Roles'
                                : interaction.customId === 'admin_bots'
                                    ? 'Administrator Bots'
                                    : 'Administrator Humans',
                            client.color
                        );

                        interaction.update({ components: [] }).then(() => msg.delete().catch(() => null));

                    });

                    collector.on('end', () => {
                        if (!msg.deleted) msg.edit({ components: [] }).catch(() => null);
                    });
...
function paginate(message, items, listType, color) {
    ...
    message.channel.send({
        ...
        collector.on('collect', interaction => {
            ...
            if (interaction.customId === 'start') currentPage = 0;
            else if (interaction.customId === 'prev') currentPage--;
            else if (interaction.customId === 'next') currentPage++;
            else if (interaction.customId === 'end') currentPage = totalPages - 1;
            else if (interaction.customId === 'delete') return msg.delete().catch(() => null);
